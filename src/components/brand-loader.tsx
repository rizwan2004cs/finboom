// FinBoom logo loader, shared by route loading states. `fullscreen` covers
// the whole viewport (root-level loads); without it the loader centers inside
// the current layout (e.g. the dashboard shell's content area).
export function BrandLoader({ fullscreen = false }: { fullscreen?: boolean }) {
  return (
    <div
      className={
        fullscreen
          ? "fixed inset-0 z-[9999] flex items-center justify-center bg-[#f8f8fa] dark:bg-[#1c1c1e] transition-colors"
